...
'''
For the print statements that are standing in for breakpoints to allow me to see the flow of the code more generally
than the execution stack, I used the following format:
=== < 1 word for the function name> - <Area of function> <number for which of that same tiype it is, if needed> ===
for example: === BATTERY - MID 2 === means:
    the second print statement in the middle of the battery function

    Note: There will only ever be 1 BEGIN statement and it will be the first line of the function.
    Also, The END print statements come as either the last line before a return or yield, or the last line of a function
    MID prints are anything in between BEGIN and END

sadly, there is no way to view values of code at runtime without adding a ton more print statements, but I think thats unavoidable... 
'''
